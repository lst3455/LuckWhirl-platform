...
public class StrategyArmoryDispatchTest {
    ...
    public void test_strategyArmory(){
        strategyArmoryDispatch.assembleRaffleStrategy(10002L);
        log.info("awardTable size: {} - int",iRedisService.getMap(Constants.RedisKey.STRATEGY_RATE_TABLE_KEY + 10001L).size());
        Map<Long,Long> map = iRedisService.getMap(Constants.RedisKey.STRATEGY_RATE_TABLE_KEY + 10002L);
        System.out.println(map.get(1L));
        Long awardId = iRedisService.getFromMap(Constants.RedisKey.STRATEGY_RATE_TABLE_KEY + 10002L,1L);
    ...
    public void test_getRandomAwardId(){
        for (int i = 0; i < 500; i++) {
            log.info("test result: {} - awardId", strategyArmoryDispatch.getRandomAwardId(10002L));
        }
    }
